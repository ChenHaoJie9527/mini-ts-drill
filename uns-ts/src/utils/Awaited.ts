 ...
 *  6. 返回never
 */
type IsAwaited<T> = T extends null | undefined
  ? T
...
type AwaitedRes = IsAwaited<Promise<Promise<{ name: string }>>>; // {name: string}
...
type Res = IsAwaited<typeof hasThenObj>; // 提取 onfulfilled 函数类型的第一个参数类型 value
